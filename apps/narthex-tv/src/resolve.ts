...
export async function resolveNow(pool: Pool, at: Date): Promise<NowPlaying> {
  ...
  if (resolution.entry) {
    ...
    if (playlist && !playlist.archived) {
      return { resolution, playlist, source: "schedule", settings };
    }
  }
  ...
  if (fallback && !fallback.archived) {
    return { resolution, playlist: fallback, source: "default", settings };
  }
  return { resolution, playlist: null, source: "none", settings };
}
...
export async function buildPlan(
  ...
  const { resolution, playlist, source, settings } = await resolveNow(pool, opts.at);
  ...
  const skeleton = {
    ...
    sourceKey: `${source}:${resolution.entry?.id ?? 0}:${playlist?.id ?? 0}:${
